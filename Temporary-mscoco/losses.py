...
class iSogCLR_Loss(nn.Module):
    def __init__(self, N=2900000, gamma=0.8, tau_init=0.01, world_size=8, bsz=128, rho=8.0):      
        ...
        #Inputs:
        #   N is number of samples in training set
        N = 13000000 # to get rid of the index mis-match issue
...
class TempGenerator(torch.nn.Module):  # design 3
    def __init__(self, feature_dim, M=256, tau_min=0.001, dropout_rate=0.5, rho=6.0):
        ...
        self.linear_2 = nn.Linear(self.M, 1, bias=False)
        self.linear_2.weight.data.fill_(1.0)
    ...
    def forward(self, x, return_weights=False):
        ...
        weights = nn.Softmax(dim=1)((x/self.scaler.exp()) @ normed_protos.t())
        sims = torch.sigmoid(x @ normed_protos.t())

        tau = (self.linear_1(weights * sims) - self.linear_2(sims / self.M)).squeeze() / self.rho

        if return_weights:
            return tau.clamp_(min=self.tau_min), weights
        else:
            return tau.clamp_(min=self.tau_min)




class iSogCLR_TempNet_Loss(nn.Module):  # using TempGenerator
    def __init__(self, N=2900000, gamma=0.8, world_size=4, 
                       rho=8.0, feature_dim=256, tau_min=0.01, bsz = 256):  # use temperature network      
        ...
        self.gamma_schedule = "cosine"
        self.gamma_decay_epochs = 20
        self.eps = 1e-14
        self.eta_init = 1e-5  

        self.rho = rho
        self.batch_size = bsz
        self.image_temp_gen = TempGenerator(feature_dim=feature_dim, M = self.batch_size, rho=self.rho).cuda()
        self.text_temp_gen = TempGenerator(feature_dim=feature_dim, M = self.batch_size, rho=self.rho).cuda()
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch, max_epoch):
        ...
        #print('Image Features',image_features.detach())
        ...
        # update b
        assert (image_ids >= 0).all() and (image_ids < self.b_I.shape[0]).all(), \
            f"Out of bounds: image_ids={image_ids}, b_I.shape={self.b_I.shape}"
        old_b_I = self.b_I[image_ids]
        new_b_I = torch.max(image_diffs_d_temps, old_b_I[:, None].tile(1, batch_size))
        self.b_I[image_ids] = torch.max(new_b_I, dim=1)[0]
        ...
        #print('self.b_I',self.b_I)
        #print('self.b_T',self.b_T)
        ...
        #print('exp_image_diffs',exp_image_diffs)
        #print('exp_text_diffs',exp_text_diffs)
        ...
        #print('S_I',s_I)
        #print('S_T',s_T)
        #print('weights_image',weights_image)
        #print('weights_text',weights_text)
        image_loss = torch.sum(weights_image * image_diffs, dim=1, keepdim=True)
        text_loss = torch.sum(weights_text * text_diffs, dim=0, keepdim=True)
        ...
        temp_weight_image = torch.log(s_I / (batch_size-1)) + self.b_I[image_ids][:, None] + self.rho - torch.sum(weights_image * image_diffs_d_temps, dim=1, keepdim=True)
        #print('temp_weight_image is ',temp_weight_image)
        temp_weight_text = torch.log(s_T / (batch_size-1)) + self.b_T[text_ids][None, :] + self.rho - torch.sum(weights_text * text_diffs_d_temps, dim=0, keepdim=True)
        #print('temp_weight_text is ',temp_weight_text)
        temp_image_loss = torch.mean(temp_weight_image * tau_image[:, None])
        #print('temp_image_loss is ',temp_image_loss)
        temp_text_loss = torch.mean(temp_weight_text * tau_text[None, :])
        #print('temp_text_loss is ',temp_text_loss)

        temp_loss = temp_image_loss + temp_text_loss
        #print('temp_loss',temp_loss)

        return (clip_loss, temp_loss), tau_image.cpu().detach().numpy(), tau_text.cpu().detach().numpy(), diag_sim.cpu().detach().numpy()
