...
import utils
import shutil
from dataset import create_train_dataset, create_val_dataset, create_sampler, create_train_loader, create_val_loader, create_train_dataset_for_cc12m
...
def train(model, data_loader, optimizer, optimizer_tempnet, tokenizer, epoch, max_epoch, warmup_steps, device, scheduler, grad_scaler, args):
    ...
    if args.ita_type == 'isogclr_tempnet' and epoch == args.epochs-1 and args.data == 'cc12m':
        image_tau_array = np.zeros(13000000)
        text_tau_array = np.zeros(13000000)
        scores_per_tau = np.zeros(13000000)
    
    if args.ita_type == 'isogclr_tempnet' and epoch == args.epochs-1 and args.data == 'dfn_med':
        image_tau_array = np.zeros(19280000)
        text_tau_array = np.zeros(19280000)
        scores_per_tau = np.zeros(19280000)
    ...
    for i,(image, text, idx, text_idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if grad_scaler is None:
            ...
            if args.ita_type == 'isogclr_tempnet' and epoch == args.epochs-1:
                ...
                # To get the scores for each sample
                scores_per_tau[info_dict['image_ids']] = info_dict['hardness_aware_score']
    ...
    if args.ita_type == 'isogclr_tempnet' and epoch == args.epochs-1:
        ...
        with open(os.path.join(args.output_dir, "tau.pkl"), "wb") as f:
            pickle.dump({"tau_image":image_tau_array, "tau_text":text_tau_array, "score_sample":scores_per_tau}, f, protocol=pickle.HIGHEST_PROTOCOL)

        print("image tau mean:", np.mean(image_tau_array))
        print("text tau mean:", np.mean(text_tau_array))
        print("sample score mean:", np.mean(scores_per_tau))
...
def zeroshot_transfer(model, data_loader, dataset_name, tokenizer, device):
    ...
    for c in classes:
        ...
        text_inputs = tokenizer(texts, context_length=77).to(device) 
        if args.text_encoder == 'Transformer':
            #text_outputs = model.text_encoder(text_inputs.input_ids, output_hidden_states=False)  
        ...
        text_embeds = F.normalize(text_outputs.last_hidden_state[:,0,:], dim=-1)
...
def evaluation(model, data_loader, tokenizer, device, args):
    ...
    for i in range(0, num_text, text_bs):
        ...
        if args.text_encoder == 'Transformer':
            text_output = model.text_encoder(text_input)  
            ...
            text_embed = F.normalize(text_output.last_hidden_state[:,0,:], dim=-1)
    ...
    for image, img_id in data_loader: 
        ...
        image_feat = model.visual_encoder(image)        
        #image_embed = model.vision_proj(image_feat)   # we do not need it
        image_embed = image_feat       
...
def main(args):
    ######### to initialize the distributed training and devices  #########
    ...
    ########## initializing the text tokenizer ###########
    if args.text_encoder == 'roberta-large':
        #token = '---'
        tokenizer = RobertaTokenizer.from_pretrained(args.text_encoder, local_files_only=False,use_auth_token=token)
    elif args.text_encoder == 'Transformer':
        #token = '---'   
        tokenizer =  SimpleTokenizer()
    else:
        #token = '---'   
        tokenizer = AutoTokenizer.from_pretrained(args.text_encoder, local_files_only=False,use_auth_token=token)

    print('we have created Simple Tokenizer by OpenAI')

    ####### Dataset Creation CC12M  #######
    print('here we create traindataset')
    start_epoch = 0
    train_dataset = get_data(args, epoch = start_epoch, tokenizer = tokenizer)
    print('here we have created traindataset')
    ...
    # New one 
    print("here we create trainloader")
    train_loader = train_dataset["train"].dataloader
    print("we have created the trainloader for webdataset style data")
    # replacement for the data number
    ...
    model = CLIP(image_encoder=args.image_encoder, text_encoder=args.text_encoder, embed_dim=args.embed_dim, init_model=args.init_model,
                  ...
                  sinkhorn_eps=args.sinkhorn_eps, swav_temp=args.swav_temp, swav_weight=args.swav_weight, batch_size_curr = args.batch_size_train)
    ...
    if len(args.checkpoint) > 0:
        ...
        if args.ita_type == 'isogclr_tempnet':
            with torch.no_grad():
                ...
                #image_embeds = model.vision_proj(model.visual_encoder(image)) ## not we do not need any projection 
                image_embeds = model.visual_encoder(image)
                if args.text_encoder == 'Transformer':
                    text_output = model.text_encoder(text.to(device))
                    #text_embeds = model.text_proj(text_output)
                ...
                model.criterion.image_temp_gen._init_prototypes(text_embeds[:args.batch_size_train,:]) # earlier it was 256
    ...
    if args.vis_prototypes:
        ...
        with torch.no_grad():
            for item_idx in range(N):
                ...
                image_feat = F.normalize(model.visual_encoder(image), dim=-1)#.cpu().numpy()                                                                         
                ...
                if args.text_encoder == 'Transformer':
                    ...
                    text_feat = F.normalize(text_output, dim=-1)
                else:
                    text_output = model.text_encoder(text.input_ids, attention_mask=text.attention_mask, output_hidden_states=False)
                    #text_feat = F.normalize(model.text_proj(text_output.last_hidden_state[:,0,:]), dim=-1)#.cpu().numpy()
                    text_feat = F.normalize(text_output.last_hidden_state[:,0,:], dim=-1)#.cpu().numpy()
    ...
    if args.check_samples_tau:
        ...
        with torch.no_grad():
            for image, text, idx, text_idx in tqdm(train_loader):
                ...
                #image_feat = F.normalize(model.vision_proj(model.visual_encoder(image)), dim=-1)
                image_feat = F.normalize(model.visual_encoder(image), dim=-1)
                text_output = model.text_encoder(text, attention_mask=text.attention_mask, output_hidden_states=False)
    ...
    for epoch in range(0, max_epoch):
        if not args.evaluate:
            if args.distributed:
                print('Argument set for DDP as True')
                train_dataset["train"].set_epoch(0)
...
if __name__ == '__main__':
    ...
    # data path --not okay
    parser.add_argument('--data', required=True, choices=['sbu', 'cc3m', 'cc12m', 'imagenet100','dfn_med'])
    parser.add_argument('--data_path', default='/data/datasets/cc3m/cc_data')
    parser.add_argument('--train_file', default='downstream/cc3m_train_new.json')
    parser.add_argument('--train_image_root', default='cc3m')
    parser.add_argument('--val_coco_file', type=str, default='/home/grads/a/anant_mehta/TempNet/clip_train/coco_val_new.json', help='Path to validation COCO file')
    ...
    # save learned taus for isogclr_tempnet objective --okay
    parser.add_argument('--save_learned_taus', action='store_true')

    # set the noise level for imagenet100 dataset --okay
    parser.add_argument('--noise_level', default=0.0, type=float)

    # set the fraction of data used for training --okay
    parser.add_argument('--train_frac', default=1.0, type=float)

    # check samples with high/low temperature values --okay
    parser.add_argument('--check_samples_tau', action='store_true')

    # extract data from the cc3m dataset --okay
    parser.add_argument('--extract_data', action='store_true')

    # check some learnable parameters in TempNet--okay
    parser.add_argument('--check_tempnet_params', action='store_true')

    # visualize the prototypes--okay
    parser.add_argument('--vis_prototypes', action='store_true')

    # zero-shot transfer--not okay
    ...
    # arguments for bilevel tempnet --okay
    ...
    # find clusters in a dataset --okay
    ...
    ###### cc3m only ####
    #args.train_file = '/scratch/user/anant_mehta/TempNet/clip_train/cc3m_train.json'
    #args.train_image_root = '/scratch/group/optmai/datasets/cc3m'

    ##### cc12m only ####

    args.train_file = '/scratch/user/anant_mehta/TempNet/clip_train/cc12m_full_data.json'
    args.train_image_root = '/scratch/group/optmai/datasets/cc12m_webdataset/cc12m'


    args.val_coco_file = '/scratch/user/anant_mehta/TempNet/clip_train/coco_val.json'
    args.test_coco_file = '/scratch/user/anant_mehta/TempNet/clip_train/coco_test.json'
    args.coco_image_root = '/scratch/group/optmai/datasets/mscoco'
    args.val_flickr_file = '/scratch/user/anant_mehta/TempNet/clip_train/flickr30k_val.json'
    args.test_flickr_file = '/scratch/user/anant_mehta/TempNet/clip_train/flickr30k_test.json'
    args.flickr_image_root = '/scratch/group/optmai/datasets/flickr30k'
